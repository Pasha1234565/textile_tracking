...
def insert_demo_data_command():
	...
	try:
		...
		if not frappe.db.sql("SELECT name FROM `tabJob Contractor` LIMIT 1"):
			...
			_create_demo_job_work_returns()
			frappe.db.commit()

			_create_demo_additional_fwl()
			frappe.db.commit()

			_create_demo_additional_cutting_plans()
			frappe.db.commit()
		...
		# Phase 3: Enhanced demo data for reports (runs even if Phase 1 skipped)
		# We check if report-specific data exists (FWLs with month spread)
		try:
			additional_check = frappe.db.sql(
				"SELECT name FROM `tabFabric Wastage Log` WHERE date_logged < '2026-06-01' LIMIT 1"
			)
			if not additional_check:
				print("Inserting enhanced demo data for reports...")
				_create_demo_additional_fwl()
				frappe.db.commit()

				if not frappe.db.sql("SELECT name FROM `tabJob Work Return` LIMIT 1"):
					_create_demo_job_work_returns()
					frappe.db.commit()
				...
				print("✅ Enhanced demo data for reports inserted!")
			else:
				print("✅ Enhanced demo data already exists, skipping Phase 3")
		except Exception as e:
		print(f"⏭️  Enhanced report data skipped: {e}")
...
def _create_demo_job_work_returns():
	"""Insert demo Job Work Returns linked to existing JWOs.

	The True Cost Per Piece report LEFT JOINs tabJob Work Return,
	so without returns the report shows zero received qty.
	"""
	now = now_datetime()

	jwos = frappe.db.sql(
		"SELECT name, contractor, qty_sent FROM `tabJob Work Order` ORDER BY name",
		as_dict=True,
	)
	if not jwos:
		print("  ⚠️  No JWOs found for returns")
		return

	for idx, jwo in enumerate(jwos, 1):
		received = jwo.qty_sent * 0.92  # Assume 92% received
		wastage = jwo.qty_sent * 0.08   # 8% wastage

		try:
			frappe.db.sql("""
				INSERT INTO `tabJob Work Return`
					(name, parent, parenttype, parentfield, idx,
					 qty_received, wastage_qty, wastage_pct, return_date,
					 creation, modified, modified_by, owner, docstatus)
				VALUES
					(%(name)s, %(parent)s, 'Job Work Order', 'job_work_returns', 1,
					 %(received)s, %(wastage)s, %(wastage_pct)s, %(date)s,
					 %(now)s, %(now)s, 'Administrator', 'Administrator', 0)
			""", {
				"name": frappe.generate_hash("", 10),
				"parent": jwo.name,
			...
			frappe.db.rollback()
			print(f"  ⚠️  Job Work Return skipped for {jwo.name}: {e}")

	print(f"  ✅ Created Job Work Returns for {len(jwos)} JWOs")


def _create_demo_additional_fwl():
	"""Insert more Fabric Wastage Logs across different months.

	The Contractor Wastage Trend chart needs data points across multiple
	months to show a meaningful trend line. We add entries for Jan, Feb,
	Mar 2026 for each contractor.
	"""
	now = now_datetime()
	...
	if not contractors:
		print("  ⚠️  No contractors found for additional FWL")
		return

	# Create FWL entries spread across Jan-Mar 2026 for trend data
	months = [
		{"date": "2026-01-15", "sent": 400, "waste": 12},
		{"date": "2026-01-20", "sent": 350, "waste": 8},
		{"date": "2026-02-10", "sent": 450, "waste": 15},
		{"date": "2026-02-18", "sent": 380, "waste": 10},
		{"date": "2026-03-05", "sent": 500, "waste": 18},
		{"date": "2026-03-15", "sent": 420, "waste": 11},
		{"date": "2026-03-25", "sent": 320, "waste": 6},
		{"date": "2026-04-02", "sent": 480, "waste": 14},
		{"date": "2026-04-10", "sent": 550, "waste": 20},
		{"date": "2026-05-01", "sent": 360, "waste": 9},
		{"date": "2026-05-15", "sent": 440, "waste": 13},
		{"date": "2026-06-01", "sent": 390, "waste": 7},
	]

	# Get some existing JWOs to link to (reuse them)
	jwos = [r[0] for r in frappe.db.sql(
		"SELECT name FROM `tabJob Work Order` ORDER BY name LIMIT 3"
	)]

	counter = 0
	for contractor in contractors:
		for month_data in months:
			jwo = jwos[counter % len(jwos)] if jwos else None
			qty = month_data["sent"]
			waste = month_data["waste"]
			waste_pct = round((waste / qty) * 100, 2)
			counter += 1

			# Vary waste slightly per contractor for realistic data
			ctr_idx = contractors.index(contractor)
			waste_adj = waste * (1 + ctr_idx * 0.15)  # Each contractor has diff wastage
			waste_adj = round(waste_adj, 1)

			frappe.db.sql("""
				INSERT INTO `tabFabric Wastage Log`
					(name, naming_series, contractor, job_work_order,
					 date_logged, qty_sent, wastage_qty, wastage_pct,
					 wastage_category,
					 creation, modified, modified_by, owner, docstatus, idx)
				VALUES
					(%(name)s, 'FWL-DEMO-', %(contractor)s, %(jwo)s,
					 %(date)s, %(qty)s, %(waste)s, %(waste_pct)s,
					 %(cat)s,
					 %(now)s, %(now)s, 'Administrator', 'Administrator', 0, 0)
			""", {
				"name": frappe.generate_hash("", 10),
				...
				"qty": qty,
				"waste": waste_adj,
				"waste_pct": round((waste_adj / qty) * 100, 2),
				"cat": "Production Waste",
				"now": now,
			})

	print(f"  ✅ Created {counter} additional FWL entries across months")


def _create_demo_additional_cutting_plans():
	"""Insert more Cutting Plans for the Cutting Efficiency report.

	The Cutting Efficiency report needs multiple Cutting Plan records
	to show meaningful data in its table and chart.
	"""
	now = now_datetime()

	# Get fabric rolls that already exist
	rolls = frappe.db.sql(
		"SELECT name, roll_number, length_meters, width_cm, contractor "
		"FROM `tabFabric Roll` WHERE docstatus = 1 LIMIT 2",
		as_dict=True,
	)
	if not rolls:
		print("  ⚠️  No fabric rolls found for cutting plans")
		return

	for idx, roll in enumerate(rolls):
		plan_name = f"CP-DEMO-{idx + 2:04d}"  # CP-DEMO-0002, CP-DEMO-0003
		length = roll.length_meters or 480
		width = roll.width_cm or 150
		total_area = round((length * width) / 100, 2)
		fabric_used = round(total_area * 0.82, 2)   # 82% utilization
		waste_pct = round((total_area - fabric_used) / total_area * 100, 2)

		frappe.db.sql("""
			INSERT INTO `tabCutting Plan`
				(name, naming_series, cutting_plan_name, fabric_roll,
				 roll_length_meters, roll_width_cm,
				 total_fabric_used, estimated_waste_pct,
				 docstatus,
				 creation, modified, modified_by, owner, idx)
			VALUES
				(%(name)s, 'CP-DEMO-', %(plan_name)s, %(roll)s,
				 %(length)s, %(width)s,
				 %(fabric_used)s, %(waste_pct)s, 1,
				 %(now)s, %(now)s, 'Administrator', 'Administrator', %(idx)s)
		""", {
			"name": plan_name,
			"plan_name": f"Cutting Batch {idx + 2}",
			"roll": roll.name,
			"length": length,
			"width": width,
		...
		print(f"  ✅ Created Cutting Plan: {plan_name} (roll: {roll.roll_number})")
